...
medikamente = pd.DataFrame({
    ...
    "Perfusor_Dosierung_mg_h_kg": [6.0, 0.5],  # Ziel-Dosierung
    "Zusatzinfo": [
        "Standarddosierung zur Narkoseeinleitung.",
        "Wirkungseintritt langsam, auch analgetisch."
    ]
})

# Streamlit Setup
st.title("💉 Perfusor-Rechner (Spritzeninhalt → Laufgeschwindigkeit)")
...
if gewicht:
    ...
    for idx, row in medikamente.iterrows():
        with st.expander(f"🧪 {row['Medikament']}"):
            st.markdown(f"""
            **Ziel-Perfusordosis:** {row['Perfusor_Dosierung_mg_h_kg']} mg/h/kg  
            📝 *{row['Zusatzinfo']}*
            """)
            ...
            ziel_dosis_mg_h = row["Perfusor_Dosierung_mg_h_kg"] * gewicht
